...
import (
	...
	armStorage "github.com/Azure/azure-sdk-for-go/services/storage/mgmt/2019-06-01/storage"
...
func buildArmClient(ctx context.Context, config BackendConfig) (*ArmClient, error) {
	...
	sender := sender.BuildSender("backend/remote-state/stack")
...
func buildArmEnvironment(ctx context.Context, config BackendConfig) (*azure.Environment, error) {
	...
	// managementEndpoint := fmt.Sprintf("%s%s", strings.TrimSuffix(config.CustomResourceManagerEndpoint, "/"), "/metadata/endpoints?api-version=1.0")
	...
	// log.Printf("[DEBUG] ########## %v", jsonResponse)
